var bookappointment={
		...
		validateStudent:function(studentId,password){
			...
			if(!studentId||!password){ //cookie中studentid 或 password中任一为空 要求登陆
				return "nothing";
			}else {
		...
		detail:{
			...
			init:function(params){
				...
				if(!studentId||!password){
					//设置弹出层属性
					var  IdAndPasswordModal=$('#varifyModal'); //document.getElementByid('id')
		...
		appointment:function(bookId,studentId, node){
			...
			$('#appointmentBtn').one('click', function () {
				...
				//2、发送预约请求执行预约
				$.post(appointmentUrl,{},function(result){   //Ajax强大之处，向Controller方法提出请求和返回结果在一处!{}为请求参数
					if(result && result['success']){         //同时还可以连续取对象的子对象！
						...
						if(state==1){    //预约成功 显示馆藏数量-1
							var values=$('#number').text();
							$('#number').text(parseInt(values)-1);
						}
